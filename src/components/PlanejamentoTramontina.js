...
export default function PlanejamentoTramontina({ socket }) {
    ...
    const podeEditar = typeof pode === 'function' ? pode('tramontina_editar') : false;
    ...
    const [filtros, setFiltros] = useState({ uf: '', regiao: '', operacao: '', statusEmb: '', agendamento: '' });
    ...
    const carregar = useCallback(async () => {
        ...
        try {
            const [rRotas, rKpis] = await Promise.all([
                api.get(`/api/tramontina/rotas?mes=${mesRef}`),
                api.get(`/api/tramontina/kpis?mes=${mesRef}`),
            ]);
            if (rRotas.data?.success) setRotas(rRotas.data.rotas);
            if (rKpis.data?.success) setKpis(rKpis.data.kpis);
        } catch (e) {
            console.error('Erro carregar Tramontina:', e);
        } finally {
            setLoading(false);
        }
    }, [mesRef]);
    ...
    const novaRota = async () => {
        ...
        try {
            await api.post('/api/tramontina/rotas', { mes_referencia: mesRef, status_embarque: 'PROGRAMADA' });
    ...
    return (
        <div style={{ padding: '10px 0' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px', flexWrap: 'wrap' }}>
                ...
                <span style={{ fontSize: '20px', fontWeight: 700, color: '#f1f5f9' }}>Planejamento Tramontina</span>
                ...
                <div style={{ marginLeft: 'auto', display: 'flex', gap: '8px' }}>
                    {podeEditar && (
                        <>
                            <button onClick={() => setModalImportar(true)} style={btnSecundario}>
                                <Upload size={14} /> Importar XLSX
                            </button>
                            <button onClick={novaRota} style={btnPrimario}>
            ...
            <div style={{ ...glass, padding: '12px 16px', marginBottom: '16px', display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                {(filtros.operacao || filtros.statusEmb || filtros.regiao || filtros.uf || filtros.agendamento) && (
                    <button onClick={() => setFiltros({uf:'',regiao:'',operacao:'',statusEmb:'',agendamento:''})} style={{ ...btnSecundario, fontSize: '11px', padding: '4px 10px' }}>
                        Limpar
                    </button>
                )}
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px', minWidth: '1100px' }}>
                    <thead>
                        <tr style={{ background: 'rgba(255,255,255,0.03)' }}>
                            ...
                            <Th style={{ width: 100 }}>Data Prev</Th>
                            <Th style={{ width: 100 }}>Embarque</Th>
                            <Th>Motorista</Th>
                            <Th style={{ width: 100 }}>Cavalo</Th>
                            <Th style={{ width: 100 }}>Carreta</Th>
                            <Th>Redespacho</Th>
                    ...
                    <tbody>
                        {rotasFiltradas.map(rota => (
                            <RotaLinha
                                key={rota.id}
                                rota={rota}
                                expandida={expandidas.has(rota.id)}
                                onToggle={() => toggleExpandir(rota.id)}
                                onEditar={editarRota}
                                onRemover={removerRota}
                                onNovaEntrega={novaEntrega}
                                onEditarEntrega={editarEntrega}
                                onRemoverEntrega={removerEntrega}
                                onEditando={broadcastEditando}
                        ...
                        {rotasFiltradas.length === 0 && !loading && (
                            <tr><td colSpan={13} style={{ padding: '40px', textAlign: 'center', color: '#475569', fontSize: '13px' }}>
                                {rotas.length === 0 ? 'Nenhuma rota cadastrada para este mês. Crie a primeira ou importe a planilha.' : 'Nenhuma rota corresponde aos filtros.'}
                            </td></tr>
                        )}
                    </tbody>
                </table>
